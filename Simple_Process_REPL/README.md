 ...
 * `ls /path/to/something` to list the contents of a folder in the application state.
 * `help <ns>` to get help for the namespace.
 * `help <ns>/<function>` to get help for a function in the namespace.
 * `show` to browse the Application state data structure. 
...
It's really easiest to just explore SPR and it's application state with `show` 
in the REPL.
 
The command: **show** in the REPL will give the Application state to you in yaml.
The easiest way to access it is `show device` or `show config serial`
with `show key1 key2,...` is the command to find sub-section or attributes in the REPL.
`show config` , `show defaults`, or just __show__ 
...
* show - show the Application state in YAML form.
 ...
 * _show_ device
 * _show_ foo
 * _def foo "foo show" show foo
 * _partial_ bar "foo show" show foo
 * _help_ foo
 * _help_ bar
 * foo
 * bar
